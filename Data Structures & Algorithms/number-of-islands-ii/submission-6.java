...
*/
    int[] find(int[] x, int[][][] parr) {
        ...
        if (parr[i][j][0] != i || parr[i][j][1] != j) parr[i][j] = find(parr[i][j], parr);
        return parr[i][j];
    }
    ...
    int union(int[] x, int[] y, int[][][] parr) {
        ...
        if (Arrays.equals(root_x, x)) {
            // curr pos is not connected yet, let's connect and keep the same number os islands
            parr[root_x[0]][root_x[1]] = root_y; // connect with neighboor
            return 0;
        }

        if (Arrays.equals(root_x, root_y)) return 0; // keep the same number os islands

        parr[root_y[0]][root_y[1]] = root_x;
    ...
    public List<Integer> numIslands2(int m, int n, int[][] positions) {
        ...
        // (i, j) -> (parrI, parrJ)
        var parr = new int[m][n][2];
        for (var i = 0; i < m; i++) {
            for (var j = 0; j < n; j++) {
                parr[i][j] = new int[]{i, j}; // all cells are parent of itself
            }
        }
